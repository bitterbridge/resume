...
% Define the boolean variable outside of the environment to ensure it's recognized.
\newboolean{firstitem}

% Define a new list type for a comma-separated list.
\newenvironment{runlist}
{% Initialization
  \setboolean{firstitem}{true}% Flag to track the first item
  \renewcommand\item{%
              \ifthenelse{\boolean{firstitem}}%
              {\setboolean{firstitem}{false}}% First item, do not print comma, switch flag off
...
\newcommand{\resumeskillarea}[2]{%
  \item \small\textbf{#1}: \begin{runlist}#2\end{runlist}
}
